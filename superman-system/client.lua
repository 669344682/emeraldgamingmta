...
function canPlayerFly(player)
	...
	local isTrial = triggerServerEvent("superman:isTrial", player)
	if (isTrial == true) then
		outputDebugString("PERMS", 3)
	else
		outputDebugString("no perms", 3)
	end
end
addEventHandler("onClientResourceStart", getResourceRootElement(thisResource), canPlayerFly, false)]]
