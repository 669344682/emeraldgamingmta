...
local SCOREBOARD_DISABLED_CONTROLS	= {"next_weapon",	-- Controls that are disabled when the scoreboard is showing
										...
										"radio_next",
										"radio_previous"}
local SCOREBOARD_TOGGLE_TIME		= 50						-- Time in milliseconds to make the scoreboard (dis)appear
local SCOREBOARD_POSTGUI			= true						-- Set to true if it must be drawn over the GUI
local SCOREBOARD_INFO_BACKGROUND	= {75, 230, 10, 150}		-- RGBA color for the info header background
local SCOREBOARD_SERVER_NAME_COLOR  = {255, 255, 255, 230}      -- RGBA color for the server name text
local SCOREBOARD_PLAYERCOUNT_COLOR	= {255, 255, 255, 230}		-- RGBA color for the server player count text
local SCOREBOARD_BACKGROUND			= {0, 0, 0, 140}			-- RGBA color for the background
local SCOREBOARD_BACKGROUND_IMAGE	= {255, 255, 255, 180}		-- RGBA color for the background image
...
local SCOREBOARD_SCROLL_FOREGROUND  = {50, 255, 50, 160}    	-- RGBA color for the scroll foreground
...
for k = 1, #SCOREBOARD_COLUMNS_WIDTH do SCOREBOARD_COLUMNS_WIDTH[k] = math.floor (SCOREBOARD_COLUMNS_WIDTH[k] * SCOREBOARD_WIDTH) end
...
local function createPlayerCache(ignorePlayer)
	...
	table.sort (g_players, function (a, b)
		local idA = getElementData(a, "player:id") or 0
		local idB = getElementData(b, "player:id") or 0
...
local function onToggleKey (key, keyState)
	if getElementData(localPlayer, "loggedin") ~= 0 then
		if not g_scoreboardDummy then -- Check if the scoreboard element has been created
...
drawBackground = function ()
	...
	if g_currentHeight > SCOREBOARD_HEADER_HEIGHT then
		...
		dxDrawImage (SCOREBOARD_X + 40, SCOREBOARD_Y + 150,
					  SCOREBOARD_WIDTH - 100, SCOREBOARD_HEIGHT - 370,
					  ":assets/images/logoLarge.png", 0, 0, 0, SCOREBOARD_BACKGROUND_IMAGE, SCOREBOARD_POSTGUI)
		-- Overlay
...
--]]
local function drawRowBounded (id, name, account, ping, colors, font, top)
	...
	if left < right then
		dxDrawText (id, left, top, right, bottom, colors[1], 1, font, "right", "top", true, false, SCOREBOARD_POSTGUI)
		...
		if left < right then
			dxDrawText (name, left, top, right, bottom, colors[2], 1, font, "left", "top", true, false, SCOREBOARD_POSTGUI)
			
			if (getElementData(localPlayer, "staff:rank") >= 1) and (getElementData(localPlayer, "duty:staff") > 0) then
				 -- Account
				left = rowsBoundingBox[2][1] + 170
				right = clamp (0, rowsBoundingBox[2][2], maxWidth)

				if left < right then
					dxDrawText(account, left, top, right, bottom, colors[2], 1, font, "left", "top", true, false, SCOREBOARD_POSTGUI)
				end
			end
...
drawScoreboard = function ()
	...
	local serverName = getElementData (g_scoreboardDummy, "serverName") or "Emerald Gaming Roleplay"
	local maxPlayers = getElementData (g_scoreboardDummy, "maxPlayers") or 100
	serverName = tostring (serverName)
	maxPlayers = tonumber (maxPlayers)
	
	-- Render the header
	-- Calculate the bounding box for the header texts
	local left, top, right, bottom = SCOREBOARD_X - 15, SCOREBOARD_Y + 2, SCOREBOARD_X + g_currentWidth - 2, SCOREBOARD_Y + SCOREBOARD_HEADER_HEIGHT - 2
	...
	local strPlayerCount = "Players: " .. tostring(#g_players) .. "/" .. tostring(maxPlayers) .. " "
	...
	local renderEntry = function (player)
		local hasFiles = getElementData(player, "loggedin")
		if (hasFiles) then -- Ensure they are on the login panel before showing them on scoreboard.
			-- Get the player data
			...
			local playerAccount = getElementData(player, "account:username") or "None"
			playerAccount = tostring(playerAccount)
			...
			-- Render it.		
			drawRowBounded(playerID, playerName, playerAccount, playerPing, colors, "default-bold", top)
		end
	end
...
function isVisible() return g_isShowing end