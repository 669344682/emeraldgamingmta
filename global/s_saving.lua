...
local playersToSave = {}
local SAVE_TIME = 3000 -- Save timer in ms for how much time to allow before saving each element type, increase this if not all content is being saved.
...
function hourlyGlobalSave()
	...
	for i, thePlayer in ipairs(playersToSave) do
		if isElement(thePlayer) then
			...
			outputDebug("@hourlyGlobalSave: Attempted to save data on player who isn't an element.")
		end
	end

	-- Save all vehicles.
	setTimer(function() exports["vehicle-system"]:saveAllVehicles() end, SAVE_TIME * 2, 1)

	-- Save all interiors.
	setTimer(function() exports["interior-system"]:saveAllInteriors() end, SAVE_TIME * 3, 1)

	-- Save all factions.
	setTimer(function() exports["faction-system"]:saveAllFactions() end, SAVE_TIME * 4, 1)

	-- Save all teleporters.
	setTimer(function() exports["teleport-manager"]:saveAllTeleporters() end, SAVE_TIME * 5, 1)
end
...
setTimer(updateTIS, 600000, 0) -- 10 minutes.
...
function savePlayerData(thePlayer, saveType)
	...
	if (saveType == 1) or (saveType == 2) or (saveType == 3) or (saveType == 5) or (saveType == 6) then
		...
		exports.mysql:Execute(
			"UPDATE `accounts` SET `lastlogin` = (?), `muted` = (?), `reports` = (?), `warns` = (?), `emeralds` = (?) WHERE `id` = (?);",
			lastLogin, muted, reports, warns, emeralds, accountID
		)
	end
	...
	if (saveType == 1) or (saveType == 2) or (saveType == 4) or (saveType == 5) then
		...
		if (logged == 1) or (saveType == 5) and not (logged == 2) then -- Either they are currently logged in or they have just left the server.
			...
			if tonumber(characterDBID) then
				...
				arrestedtime = 0 -- Do something here. @requires prison-system/pd-system
