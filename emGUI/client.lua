...
NOTE FOR DEVELOPERS:
	Most of the code below has been heavily adjusted from the original dxGUI library by thisdp,
	he isn't a native English speaker so most variable naming conventions in this resource do
	not match that of the rest. I've done my best to clean up the code and make the changes that
	I needed for my fork of the resource, but the rest is a maze - good luck. - Skully
...
systemFont = "default"
textFont = dxCreateFont("fonts/textFont.ttf", fontSize["textFont"])
buttonFont = dxCreateFont("fonts/buttonFont.ttf", fontSize["buttonFont"])
labelFont = dxCreateFont("fonts/labelFont.ttf", fontSize["labelFont"])
tabsFont = dxCreateFont("fonts/tabsFont.ttf", fontSize["tabsFont"])
consoleFont = dxCreateFont("fonts/consoleFont.ttf", fontSize["consoleFont"])


function setSystemFont(font, size, bold, quality)
	assert(type(font) == "string","Bad argument @setSystemFont at argument 1, expect a string got "..dxGetType(font))
...
----------------------------------------------------- [Core] -----------------------------------------------------
dgsElementType = {}
BottomFatherTable = {}		-- Store save bottoms parent element.
FatherTable = {}		-- Store save parent table.
ChildrenTable = {}		-- Stores child elements.
MaxFatherTable = {}		-- Highest.
dgsElementData = {}		-- Stores element data.

function dxGetType(shenMeGui)
...
function dxGetDxGUINoParent(alwaysBottom)
	return alwaysBottom and BottomFatherTable or MaxFatherTable
end

function dxSetParent(erzi,baba,nocheckfather)
	if isElement(erzi) then
		...
		if isElement(baba) then
			if not emIsDxElement(baba) then return end
...
function emGetData(element,key)
...
function emSetData(element,key,value,check)
	local key = tostring(key)
	local emGUIType = dxGetType(element)
	if isElement(element) and emGUIType then
		...
		if not check then
			if emGUIType == "dgs-dxscrollbar" then
				if key == "length" then
					local w,h = dxGetSize(element,false)
			...
			elseif (emGUIType == "dgs-dxgridlist" or emGUIType == "dgs-dxscrollpane") then
				if key == "scrollBarThick" then
					assert(type(value) == "number","Bad argument 'emSetData' at 3,expect number got"..type(value))
					local scrollbars = dgsElementData[element]["scrollbars"]
					local size = dgsElementData[element]["absSize"]
					dxSetPosition(scrollbars[1],size[1]-value,0,false)
					dxSetSize(scrollbars[1],value,size[2]-value,false)
					dxSetPosition(scrollbars[2],0,size[2]-value,false)
					dxSetSize(scrollbars[2],size[1]-value,value,false)
					if emGUIType == "dgs-dxgridlist" then
			...
			elseif emGUIType == "dgs-dxcombobox" then
				if key == "scrollBarThick" then
					assert(type(value) == "number","Bad argument 'emSetData' at 3,expect number got"..type(value))
			...
			elseif emGUIType == "dgs-dxtabpanel" then
				...
				elseif key == "tabsidesize" then
					...
					emSetData(element,"allleng",allleng)
			...
			elseif emGUIType == "dgs-dxtab" then
				if key == "text" then
				...
				elseif key == "width" then
					...
					if absrltWidth[1] < 0 then
						...
						emSetData(tabpanel,"allleng",allleng)
					end
				elseif key == "absrltWidth" then
					--
				end
			elseif emGUIType == "dgs-dxedit" then
			...
			elseif emGUIType == "dgs-dxmemo" then
			...
			elseif emGUIType == "dgs-dxprogressbar" then
				if key == "progress" then
					triggerEvent("ondxProgressBarChange",element,value,oldValue)
				end
			end
			if key == "text" then
...
function emIsDxElement(element)
	if string.sub(dxGetType(element) or "",1,6) == "dgs-dx" then
		return true
	end
	return false
end
...
-- Data types stored for each GUI component.
emGUIType = {
	"dgs-dxbutton",
	"dgs-dxcmd",
	"dgs-dxedit",
...
-- Debugging for framework.
addCommandHandler("debugemgui",function()
	if not exports.global:isPlayerLeadDeveloper(localPlayer) then return end
	DEBUG_MODE = not getElementData(localPlayer, "emGUI-Debug")
	setElementData(localPlayer, "emgui:debug", DEBUG_MODE, false)
end)

DEBUG_MODE = getElementData(localPlayer, "emgui:debug")
...
function GUIRender()
	...
	if DEBUG_MODE then
		...
		local version = 2.0
		dxDrawText("emGUI Framework",6,sH*0.4-114,sW,sH,black)
		dxDrawText("emGUI Framework",5,sH*0.4-115)
		...
		local enterStr = MouseData.hit and dxGetType(MouseData.hit).."("..tostring(MouseData.hit)..")" or "None"
		...
		local leftStr = MouseData.clickl and dxGetType(MouseData.clickl).."("..tostring(MouseData.clickl)..")" or "None"
		local rightStr = MouseData.clickr and dxGetType(MouseData.clickr).."("..tostring(MouseData.clickr)..")" or "None"
		...
		DGSCount = 0
		for k,v in ipairs(emGUIType) do
		...
		dxDrawText("Elements Shown: "..DGSShow,11,sH*0.4-9,sW,sH,black)
		dxDrawText("Elements Shown: "..DGSShow,10,sH*0.4-10,sW,sH)
		dxDrawText("Element Count: "..DGSCount,11,sH*0.4+6,sW,sH,black)	
		dxDrawText("Element Count: "..DGSCount,10,sH*0.4+5,sW,sH)
...
-- this is exported.
function dxShowCursor(bool)
	showCursor(bool)
end

-- Main clientsided framework GUI handler.
function renderGUI(v, mx, my, enabled, rndtgt, OffsetX, OffsetY, galpha, visible)
	...
	if eleData.visible and visible and isElement(v) then
		...
		local x,y = dxGetPosition(v,false,true)
		...
		if dxType == "dgs-dxwindow" then
			if x and y then
				...
				local cursor = dgsElementData[v].cursor
				...
				local font = eleData.font or textFont -- Sets the window title font.
				local titnamecolor = eleData.titnamecolor
				titnamecolor = applyColorAlpha(titnamecolor,galpha)
				local txtSizX,txtSizY = eleData.textsize[1],eleData.textsize[2] or eleData.textsize[1]
				dxDrawText(eleData.text,x,y,x+w,y+titsize,titnamecolor,txtSizX,txtSizY,textFont,"center","center",true,false,rendSet,eleData.colorcoded)
				------------------------------------
						...
						if debugShader ~= "Error" then
							if not isElement(debugShader) then
								...
								if isElement(debugShader) then
									...
									outputChatBox("[emGUI] Couldn't create ellipse shader (May not have enough video memory or GPU isn't supported for dX.)",255,0,0)
			...
			if x and y then
				...
				if MouseData.nowShow == v then
					if getKeyState("lctrl") and getKeyState("a") then
						emSetData(v,"cursorpos",0)
						emSetData(v,"selectfrom",utf8.len(text))
					end
				end
		...
		elseif dxType == "dgs-dxmemo" then
			...
			if x and y then
				...
				if MouseData.nowShow == v then
					if getKeyState("lctrl") and getKeyState("a") then
						emSetData(v,"cursorposXY",{0,1})
						emSetData(v,"selectfrom",{utf8.len(text[allLine]),allLine})
					end
				end
		...
		elseif dxType == "dgs-dxscrollbar" then
			...
			if x and y then
				...
				local ax,ay = dxGetPosition(v,false)
				...
				if MouseData.enter == v then
					if not MouseData.clickData then
						...
						if MouseData.clickData == 2 then
							...
							emSetData(v,"position",position*100)
						end
					end
				end
				------------------------------------
			...
			if x and y then
				...
				if selected == -1 then
					...
					if isElement(rendt) then
						...
						for cid,child in ipairs(dxGetChildren(tabs[selected])) do
		...
		elseif dxType == "dgs-dxcmd" then
			...
			if x and y then
				...
				local font = eleData.font or consoleFont
				local txtSizX,txtSizY = eleData.textsize[1],eleData.textsize[2] or eleData.textsize[1]
				for i=1,#cmdtexts do
					local movex = 0
					local rndStr = ""
...
function processPositionOffset(gui,x,y,w,h,parent,rndtgt,offsetx,offsety)
	...
	local P_emGUIType = dgsElementType[parent]
	if P_emGUIType == "dgs-dxscrollpane" then
...
function checkEditCursor(button,state)
	if button == "mouse_wheel_up" or button == "mouse_wheel_down" then
		local dgsType = dxGetType(MouseData.enter)
		local scroll = button == "mouse_wheel_down" and 1 or -1
		local scrollbar = MouseData.enter
		if dxGetType(scrollbar) == "dgs-dxscrollbar" then
		...
		elseif dgsType == "dgs-dxgridlist" then
			if MouseData.enterData then
				...
				if dxGetVisible(scrollbar) then
					scrollScrollBar(scrollbar,button == "mouse_wheel_down" or false)
				end
			end
		...
		elseif isElement(MouseData.scrollPane) then
			...
			local sbr
			if dxGetVisible(scrollbar1) then
				sbr = scrollbar1
			end
			if dxGetVisible(scrollbar2) then
				sbr = scrollbar2
			end
			if sbr then
		...
		elseif dgsType == "dgs-dxtabpanel" or dgsType == "dgs-dxtab" then
			...
			if width > w then
				...
				if my < y+height then
					...
					emSetData(tabpanel,"taboffperc",orgoff)
				end
			end
	...
	if dxGetType(MouseData.nowShow) == "dgs-dxedit" then
		...
		if state then
			...
			if button == "arrow_l" then
				dxEditMoveCaret(MouseData.nowShow,-1,shift)
			...
			elseif button == "arrow_u" then
				if dxGetType(cmd) == "dgs-dxcmd" then
					...
					if history[int+1] then
						int = int+1
						emSetData(cmd,"cmdCurrentHistory",int)
						dxSetText(MouseData.nowShow,history[int])
						dxEditSetCaretPosition(MouseData.nowShow,#history[int])
					end
				end
			...
			elseif button == "home" then
				dxEditSetCaretPosition(MouseData.nowShow,0,getKeyState("lshift") or getKeyState("rshift"))
			elseif button == "end" then
				dxEditSetCaretPosition(MouseData.nowShow,#text,getKeyState("lshift") or getKeyState("rshift"))
	...
	elseif dxGetType(MouseData.nowShow) == "dgs-dxmemo" then
		if state then
			local cmd = dgsElementData[MouseData.nowShow].mycmd
			local shift = getKeyState("lshift") or getKeyState("rshift")
			if button == "arrow_l" then
				dxMemoMoveCaret(MouseData.nowShow,-1,0,shift)
			...
			elseif button == "arrow_u" then
				dxMemoMoveCaret(MouseData.nowShow,0,-1,shift,true)
				if isTimer(MouseData.Timer["memoMove"]) then
					killTimer(MouseData.Timer["memoMove"])
				end
				if isTimer(MouseData.Timer["memoMoveDelay"]) then
			...
			elseif button == "home" then
				...
				dxMemoSetCaretPosition(MouseData.nowShow,0,tarline,getKeyState("lshift") or getKeyState("rshift"))
			elseif button == "end" then
				...
				dxMemoSetCaretPosition(MouseData.nowShow,utf8.len(text[line] or ""),tarline,getKeyState("lshift") or getKeyState("rshift"))
			elseif button == "delete" then
				...
				if cpos[1] ~= spos[1] or cpos[2] ~= spos[2] then
					dxMemoDeleteText(MouseData.nowShow,cpos[1],cpos[2],spos[1],spos[2])
					dgsElementData[MouseData.nowShow].selectfrom = dgsElementData[MouseData.nowShow].cursorposXY
				else
					local tarindex,tarline = dxMemoSeekPosition(dgsElementData[MouseData.nowShow].text,cpos[1]+1,cpos[2])
					dxMemoDeleteText(MouseData.nowShow,cpos[1],cpos[2],tarindex,tarline)
					MouseData.Timer["memoMoveDelay"] = setTimer(function()
						if dxGetType(MouseData.nowShow) == "dgs-dxmemo" then
							MouseData.Timer["memoMove"] = setTimer(function()
								if dxGetType(MouseData.nowShow) == "dgs-dxmemo" then
			...
			elseif button == "backspace" then
				...
				if cpos[1] ~= spos[1] or cpos[2] ~= spos[2] then
					...
					local tarindex,tarline = dxMemoSeekPosition(dgsElementData[MouseData.nowShow].text,cpos[1]-1,cpos[2])
					dxMemoDeleteText(MouseData.nowShow,tarindex,tarline,cpos[1],cpos[2])
					MouseData.Timer["memoMoveDelay"] = setTimer(function()
						if dxGetType(MouseData.nowShow) == "dgs-dxmemo" then
							MouseData.Timer["memoMove"] = setTimer(function()
								if dxGetType(MouseData.nowShow) == "dgs-dxmemo" then
			...
			elseif button == "c" or button == "x" then
				if getKeyState("lctrl") or getKeyState("rctrl") then
					local cpos = dgsElementData[MouseData.nowShow].cursorposXY
					local spos = dgsElementData[MouseData.nowShow].selectfrom
					local theText = dxMemoGetPartOfText(MouseData.nowShow,cpos[1],cpos[2],spos[1],spos[2],button == "x")
			...
			if isElement(edit) then
				if MouseData.nowShow == edit then
					if dgsElementData[edit].clearSelection then
						emSetData(edit,"selectfrom",dgsElementData[edit].cursorpos)
					end
					MouseData.nowShow = false
				end
			end
			...
			if isElement(memo) then
				if MouseData.nowShow == memo then
					if dgsElementData[memo].clearSelection then
						emSetData(memo,"selectfrom",dgsElementData[memo].cursorposXY)
					end
					MouseData.nowShow = false
				end
			end
...
function scrollScrollBar(scrollbar,button)
	...
	emSetData(scrollbar,"position",(gpos < 0 and 0) or (gpos >100 and 100) or gpos)
end
...
addEventHandler("onDgsTextChange",root,function(text)
	...
	if isElement(gui) then
		...
		if isElement(parent) then
			if dxGetType(parent) == "dgs-dxcmd" then
				...
				if history[hisid] ~= text then
					emSetData(parent,"cmdCurrentHistory",0)
				end
			end
		end
	end
	...
	if guitype == "gui-edit" then
		...
		if isElement(myedit) then
			if source == dgsElementData[myedit].edit then
				...
				emSetData(myedit,"text",text_new)
				...
				if getKeyState("delete") then
					if sele ~= 0 then
						if sele > 0 then
							dxEditSetCaretPosition(myedit,from-sele)
						else
							dxEditSetCaretPosition(myedit,from)
						end
					end
				elseif getKeyState("backspace") then
					if sele == 0 then
						dxEditSetCaretPosition(myedit,pos+utf8.len(text_new)-utf8.len(text_old))
					else
						if sele > 0 then
							dxEditSetCaretPosition(myedit,pos)
						else
							dxEditSetCaretPosition(myedit,pos+utf8.len(text_new)-utf8.len(text_old))
						end
					end
				else
					dxEditSetCaretPosition(myedit,pos+offset)
				end
				local pos = dgsElementData[myedit].cursorpos
				if pos > utf8.len(text_new) then
					dxEditSetCaretPosition(myedit,utf8.len(text_new))
				end
			end
		end
	elseif guitype == "gui-memo" then
		...
		if isElement(mymemo) then
			if source == dgsElementData[mymemo].memo then
				...
				if #text ~= 0 and not cool then
					...
					dxMemoDeleteText(mymemo,cursorposXY[1],cursorposXY[2],selectfrom[1],selectfrom[2])
...
function dgsCheckHit(hits,mx,my)
	if not isElement(MouseData.clickl) or not (dxGetType(MouseData.clickl) == "dgs-dxscrollbar" and MouseData.clickData == 2) then
		if MouseData.enter ~= hits then
			if isElement(MouseData.enter) then
				triggerEvent("onDgsMouseLeave",MouseData.enter,mx,my,hits)
				if dxGetType(MouseData.clickl) == "dgs-dxscrollbar" then
				...
				if dxGetType(MouseData.enter) == "dgs-dxgridlist" then
					emSetData(MouseData.enter,"preSelect",{-1,-1})
				end
			end
	...
	if isElement(MouseData.clickl) then
		...
		if MouseData.Move then
			if dxGetType(MouseData.clickl) == "dgs-dxwindow" then
...
addEventHandler("onDgsMouseClick",root,function(button,state)
	...
	local guitype = dxGetType(source)
	if state == "down" then
		dxBringToFront(source,button)
		...
		if button == "left" then
			...
			if guitype == "dgs-dxscrollbar" then
				...
				local w,h = dxGetSize(source,false)
				...
				if scrollArrow then
					if MouseData.clickData == 1 then
						...
						emSetData(source,"position",(gpos < 0 and 0) or (gpos >100 and 100) or gpos)
			...
			elseif guitype == "dgs-dxgridlist" then
				...
				if dgsElementData[source].sortEnabled then
					...
					if column and column >= 1 then
						...
						dxGridListSetSortFunction(source,targetfunction)
						dxGridListSetSortColumn(source,column)
					end
				end
				--------
					...
					if #preSelect == 2 then
						if selectionMode == 1 then
							if multiSelection then
								if ctrl then
									local selected = dxGridListItemIsSelected(source,preSelect[1],1)
									dxGridListSelectItem(source,preSelect[1],1,not selected)
								elseif shift then
									if clicked and #clicked == 2 then
										dxGridListSetSelectedItem(source,-1,-1)
										local startRow,endRow = math.min(clicked[1],preSelect[1]),math.max(clicked[1],preSelect[1])
										for row = startRow,endRow do
											dxGridListSelectItem(source,row,1,true)
										end
										dgsElementData[source].itemClick = clicked
									end
								else
									dxGridListSetSelectedItem(source,preSelect[1],1)
						...
						elseif selectionMode == 2 then
							if multiSelection then
								if ctrl then
									local selected = dxGridListItemIsSelected(source,1,preSelect[2])
									dxGridListSelectItem(source,1,preSelect[2],not selected)
								elseif shift then
									if clicked and #clicked == 2 then
										dxGridListSetSelectedItem(source,-1,-1)
										...
										for column = startColumn, endColumn do
											dxGridListSelectItem(source,1,column,true)
										end
										dgsElementData[source].itemClick = clicked
									end
								else
									dxGridListSetSelectedItem(source,1,preSelect[2])
									dgsElementData[source].itemClick = preSelect
								end
							else
								dxGridListSetSelectedItem(source,1,preSelect[2])
						...
						elseif selectionMode == 3 then
							if multiSelection then
								if ctrl then
									local selected = dxGridListItemIsSelected(source,preSelect[1],preSelect[2])
									dxGridListSelectItem(source,preSelect[1],preSelect[2],not selected)
								elseif shift then
									if clicked and #clicked == 2 then
										dxGridListSetSelectedItem(source,-1,-1)
										local startRow,endRow = math.min(clicked[1],preSelect[1]),math.max(clicked[1],preSelect[1])
										...
										for row = startRow,endRow do
											for column = startColumn, endColumn do
												dxGridListSelectItem(source,row,column,true)
											end
										end
										dgsElementData[source].itemClick = clicked
									end
								else
									dxGridListSetSelectedItem(source,preSelect[1],preSelect[2])
									dgsElementData[source].itemClick = preSelect
								end
							else
								dxGridListSetSelectedItem(source,preSelect[1],preSelect[2])
			...
			elseif guitype == "dgs-dxtab" then
				...
				if dgsElementData[tabpanel]["preSelect"] ~= -1 then
					emSetData(tabpanel,"selected",dgsElementData[tabpanel]["preSelect"])
				end
			elseif guitype == "dgs-dxcombobox" then
				emSetData(source,"listState",dgsElementData[source].listState == 1 and -1 or 1)
			end
		end
	else
		if button == "left" then
...
addEventHandler("onClientElementDestroy",resourceRoot,function()
	local parent = dxGetParent(source) or root
	if emIsDxElement(source) then
		...
		elseif dxGetType(source) == "dgs-dxmemo" then
		...
		elseif dxGetType(source) == "dgs-dxgridlist" then
			local rentarg = dgsElementData[source].renderTarget
		...
		elseif dxGetType(source) == "dgs-dxscrollpane" then
		...
		elseif dxGetType(source) == "dgs-dxtabpanel" then
			local rentarg = dgsElementData[source].renderTarget
		...
		elseif dxGetType(source) == "dgs-dxcombobox" then
			local rentarg = dgsElementData[source].renderTarget
			if isElement(rentarg) then
				destroyElement(rentarg)
			end
...
function checkMove()
	...
	if dxGetType(source) == "dgs-dxwindow" then
...
addEventHandler("onClientClick",root,function(button,state,x,y)
	...
	if isElement(guiele) then
		...
		if GirdListDoubleClick[state] and isTimer(GirdListDoubleClick[state].timer) then
			...
			if dxGetType(guiele) == "dgs-dxgridlist" and GirdListDoubleClick[state].gridlist == guiele and GirdListDoubleClick[state].but == button then
				local pass = true
				if selectionMode == 1 then
				...
				if pass then
					triggerEvent("ondxGridListItemDoubleClick",guiele,GirdListDoubleClick[state].but,state,clicked[1],clicked[2])
				end
			end
			...
			if dxGetType(guiele) == "dgs-dxgridlist" then
				local clicked = dgsElementData[guiele].itemClick
	...
	elseif state == "down" then
		local emGUIType = dxGetType(MouseData.nowShow)
		if emGUIType == "dgs-dxedit" or emGUIType == "dgs-dxmemo" then
			blurEditMemo()
		end
		MouseData.nowShow = false
...
function simulationClick(guiele,button)
	local x,y = dxGetPosition(guiele,false)
	local sx,sy = dxGetSize(guiele,false)
...
function dxGetMouseEnterGUI()
	return MouseData.enter
end

function dxGetMouseLeaveGUI()
...
function dgsRunString(func,...)
	...
	assert(type(fnc) == "function","[emGUI] Failed to load, bad function provided @dgsRunString")
	return fnc(...)
end
