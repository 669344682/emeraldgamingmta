...
function showRefuelGUI(vehicleFuel, fuelStation, aTankSize)
	...
	local vehicleFuelLevel = (vehicleFuel * 100) / aTankSize
	emGUI:dxProgressBarSetProgress(fuelProgressBar, vehicleFuelLevel)
	...
	addEventHandler("onClientDgsDxWindowClose", root, function()
		...
		hasRefueled, fuel, totalRefueled, fuelType, tankSize = false, 0, 0, 1, 100
...
function handleRefueling(button, press)
	if (button == "space") then
		if (press) and not isChatBoxInputActive() then
			...
			if not (refuelingSound) and (fuel <= tankSize) then
		...
		elseif not press and not isChatBoxInputActive() then
			...
			if (fuel < tankSize) then emGUI:dxSetText(stateButton, "Idle") end
...
function increaseFuelBar()
	...
	if (fuel <= tankSize) then
		if (fuel > (0.15 * tankSize)) then
			emGUI:dxSetVisible(emptyFuelLabel, false)
		end
		...
		local fuelingProgress = (fuel * 100) / tankSize
		emGUI:dxProgressBarSetProgress(fuelProgressBar, fuelingProgress)
	else
		if (refuelingSound) then stopSound(refuelingSound); refuelingSound = false end
		removeEventHandler("onClientRender", root, increaseFuelBar)
